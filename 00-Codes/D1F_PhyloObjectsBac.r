...
# Part F: copy number corrected absolute data phylo object
        # contains the absolute abundance of the 16S rRNA gene sequencing
        # agglomerated to either species or genus level
...
##########################################################################
##########################################################################
# PART F1: copy number corrected absolute data phylo object species
##########################################################################
phyabs <- readRDS("./03-Routputfiles/D_Phyloobjects/D1D_bac_abs_nocontrols_nospike.rds")
phyabsagg <- tax_glom(phyabs, taxrank = "Species")
# get and transpose otu table
otufabst <- data.frame(t(otu_table(phyabsagg)))
...
##########################################################################
# get meta data
metafabs <- data.frame(sample_data(phyabsagg))
...
##########################################################################
# get taxonomy
taxx <- data.frame(tax_table(phyabsagg))
taxx$sseqid <- rownames(taxx)
# remove X at the beginning of taxx$sseqid if there is one
taxx$sseqid <- gsub("^X", "", taxx$sseqid)
...
# save phycorr
saveRDS(phycorr, "./03-Routputfiles/D_Phyloobjects/D1F_bac_abscnc_nc_ns_species.rds")
##########################################################################
##########################################################################
##########################################################################
# PART F2: copy number corrected absolute data phylo object Genus
##########################################################################
phyabs <- readRDS("./03-Routputfiles/D_Phyloobjects/D1D_bac_abs_nocontrols_nospike.rds")
phyabsagg <- tax_glom(phyabs, taxrank = "Genus")
# get and transpose otu table
otufabst <- data.frame(t(otu_table(phyabsagg)))
# colnames of data frame can not start with a number
# add an X to all colnames of otufabst if there is not alread an X
...
taxx1$mean[taxx1$rank == "superkingdom"] <- 5
# correct name for Unassigned
taxx1$name[taxx1$Genus == "Unassigned"] <- "Unassigned"
# correct copy number for Unassigned to 5
taxx1$mean[taxx1$name == "Unassigned"] <- 5
##########################################################################
# make a column called label in taxx2
...
tax_table(phycorr) <- tax_table(as.matrix(taxx1))
# save phycorr
saveRDS(phycorr, "./03-Routputfiles/D_Phyloobjects/D1F_bac_abscnc_nc_ns_genus.rds")
##########################################################################
##########################################################################
