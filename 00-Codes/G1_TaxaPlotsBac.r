...
##########################################################################
# phylo object
phyabscorr <- readRDS("./03-Routputfiles/D_Phyloobjects/D1E_bac_abscnc_nocontrols_nospike.rds")
##########################################################################
# Prepare data
##########################################################################
# transpose otufabs
...
frequency <- read.table("./03-Routputfiles/F_blastn_bac_seqvsiso_frequencies.txt", sep = "\t", header = TRUE)
...
saveRDS(dfmeanlong, "./03-Routputfiles/G1-dfmeanlong.rds")
##########################################################################
owncolgen <- readRDS("./03-Routputfiles/Z_Helperfiles/owncolgenbac.rds")
dfmeanlong$generataxaplot <- ifelse(dfmeanlong$Genus %in% owncolgen, dfmeanlong$Genus, "Others")
# further corrections
dfmeanlong$generataxaplot <- ifelse(dfmeanlong$generataxaplot == "Bacteria", "Others", dfmeanlong$generataxaplot)
dfmeanlong$generataxaplot <- ifelse(dfmeanlong$generataxaplot == "Enterobacteriaceae", "unclassified Enterobacteriaceae", dfmeanlong$generataxaplot)
dfmeanlong$generataxaplot <- ifelse(dfmeanlong$generataxaplot == "Micrococcaceae", "unclassified Micrococcaceae", dfmeanlong$generataxaplot)
dfmeanlong$generataxaplot <- ifelse(dfmeanlong$generataxaplot == "Proteobacteria", "Others", dfmeanlong$generataxaplot)
# for this plot, change the Order of Brochothrix to Bacillales, since it is not Lactobacillales anymore
dfmeanlong$Order <- ifelse(dfmeanlong$Genus == "Brochothrix", "Bacillales", dfmeanlong$Order)
##########################################################################
...
llsplit1 <- ifelse(grepl("unclassified", legendlabel), "unclassified", "")
llsplit2 <- legendlabel
llsplit2 <-gsub("unclassified ", "", llsplit2)
...
p5 <- ggplot(dfmeanlong, aes(factor(days), proportion * total, fill = reorder(generataxaplot, index)),
  ...
  xlab("days") +
  ylab("log10(estimated genomic equivalents)")

# Display the plot
print(p5)
# save plot as tiff 25x25cm
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_mean_time.svg", p5,
       width = 20, height = 20, units = "cm")
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_mean_time.png", p5,
       width = 20, height = 20, units = "cm")
saveRDS(p5, "./03-Routputfiles/G_Taxaplots/G1_bac_mean_time.rds")
###############################################################################
# Individual Plotting
###############################################################################
# transform dfmean to long format
...
# further corrections
dflong$generataxaplot <- ifelse(dflong$generataxaplot == "Bacteria", "Others", dflong$generataxaplot)
dflong$generataxaplot <- ifelse(dflong$generataxaplot == "Enterobacteriaceae", "unclassified Enterobacteriaceae", dflong$generataxaplot)
dflong$generataxaplot <- ifelse(dflong$generataxaplot == "Micrococcaceae", "unclassified Micrococcaceae", dflong$generataxaplot)
dflong$generataxaplot <- ifelse(dflong$generataxaplot == "Proteobacteria", "Others", dflong$generataxaplot)

# for this plot, change the Order of Brochothrix to Bacillales, since it is not Lactobacillales anymore
dflong$Order <- ifelse(dflong$Genus == "Brochothrix", "Bacillales", dflong$Order)
##########################################################################
...
final_plot <- ggdraw(final_grid_with_margins) +
  ...
  draw_label("log10(estimated genomic equivalents)", x = 0.03, y = 0.5, angle = 90, hjust = 0.5, vjust = 1)


#print(final_plot)
# save plot as svg
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_individual_piece.svg", final_plot,
       width = 23.1, height = 32.45, units = "cm")
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_individual_piece.png", final_plot,
       width = 23.1, height = 32.45, units = "cm")
##########################################################################
# FOR INDIVIDUAL TIMEPOINT PLOTS PER MEAT PIECE
# split up the data on slice
plot_list <- list()
...
cumsum <- dflongsub %>%
  ...
  summarise(total = log10(sum(mean, na.rm = TRUE)))
cumsum <- data.frame(cumsum)
# calculate the proportion of each ASV per day and slice
dflongsub <- dflongsub %>%
  group_by(slice) %>%
  mutate(total_value = sum(mean, na.rm = TRUE)) %>%
  ungroup() %>%
...
final_plot <- ggdraw(final_grid_with_margins) +
  ...
  draw_label("log10(estimated genomic equivalents)", x = 0.03, y = 0.65, angle = 90, hjust = 0.5, vjust = 1)


#print(final_plot)
# save plot as svg
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_individual_days.svg", final_plot,
       width = 21, height = 29.5, units = "cm")
ggsave("./03-Routputfiles/G_Taxaplots/G1_bac_individual_days.png", final_plot,
       width = 21, height = 29.5, units = "cm")
