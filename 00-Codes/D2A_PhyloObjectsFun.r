...
library(ape)
##########################################################################
# PART B: Create Raw Data Phyloseq Object
##########################################################################
...
##########################################################################
##########################################################################
# keep only samples with 18S in target
...
###########################################################################
# import asv table
asv <- read.table("./04-asvtable/03-final/exported-table/feature-table.txt",
...
# these are negative controls and 1 SAMPLE FR-B8-06_111828-E09!!! all reads were
...
colnames(taxonomy_df) <- c("Kingdom", "Phylum", "Class",
...
matches <- read.table("./08-blastdb/comparison.mamfilt.blastout", sep = "\t",
                      header = FALSE)
...
matches <- matches[matches$length > 300, ]
# order by qseqid
...
# add additionally the taxonomy from the matching isolates for better resolution
isotax <- read.table("C:/users/rochf/Desktop/BeiRip/03-Routputfiles/B_SangerSeq/B3_ListOfDoneIsolates.txt", sep = ";",
                     header = TRUE) 
isotax <- isotax %>%
  filter(Kingdom == "Eukaryota") %>%
  select(ids, seqlength, Kingdom, Phylum, Class, Order, Family, Genus)
colnames(isotax) <- c("qseqid", "seqlength", "isoKingdom", "isoPhylum",
                  "isoClass", "isoOrder", "isoFamily", "isoGenus")
taxonomy_df <- plyr::join(taxonomy_df, isotax, by = "qseqid", type = "left")
taxonomy_df$qseqid <- gsub(".EK528", "", taxonomy_df$qseqid)
rownames(taxonomy_df) <- taxonomy_df$sseqid
# replace NA by isolate taxonomy if present
taxonomy_df$Kingdom <- ifelse(is.na(taxonomy_df$Kingdom),
                              ifelse(!is.na(taxonomy_df$isoKingdom),
                                     paste0(taxonomy_df$isoKingdom, "*"),
                                     taxonomy_df$Kingdom), taxonomy_df$Kingdom)
taxonomy_df$Phylum <- ifelse(is.na(taxonomy_df$Phylum),
                              ifelse(!is.na(taxonomy_df$isoPhylum),
                                      paste0(taxonomy_df$isoPhylum, "*"),
                                      taxonomy_df$Phylum), taxonomy_df$Phylum)
taxonomy_df$Class <- ifelse(is.na(taxonomy_df$Class),
                              ifelse(!is.na(taxonomy_df$isoClass),
                                      paste0(taxonomy_df$isoClass, "*"),
                                      taxonomy_df$Class), taxonomy_df$Class)
taxonomy_df$Order <- ifelse(is.na(taxonomy_df$Order),
                              ifelse(!is.na(taxonomy_df$isoOrder),
                                      paste0(taxonomy_df$isoOrder, "*"),
                                      taxonomy_df$Order), taxonomy_df$Order)
taxonomy_df$Family <- ifelse(is.na(taxonomy_df$Family),
                              ifelse(!is.na(taxonomy_df$isoFamily),
                                      paste0(taxonomy_df$isoFamily, "*"),
                                      taxonomy_df$Family), taxonomy_df$Family)
taxonomy_df$Genus <- ifelse(is.na(taxonomy_df$Genus),
                              ifelse(!is.na(taxonomy_df$isoGenus),
                                      paste0(taxonomy_df$isoGenus, "*"),
...
###########################################################################
setwd(owd)
saveRDS(matches, "./03-Routputfiles/D_isolatematchesfun.rds")
###########################################################################
...
# subset and save samples for Ferdl
...
saveRDS(phy_ferdl, "./03-Routputfiles/D_Phyloobjects/D2A_fun_raw_allsamples.rds")
# subset and save samples for Tea
#phy_tea <- subset_samples(phy, owner == "Tea")
#saveRDS(phy_tea, "./03-Routputfiles/Dfun_phyraw_tea.rds")
###########################################################################
###########################################################################
phy_ferdl <- subset_samples(phy_ferdl, piece != "mo" & piece != "NK")
# save phy_ferdl filtered
saveRDS(phy_ferdl, "./03-Routputfiles/D_Phyloobjects/D2A_fun_raw_nocontrols.rds")
